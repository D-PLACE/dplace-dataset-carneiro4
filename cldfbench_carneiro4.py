...
class Dataset(DatasetWithSocieties):
    ...
    id = "dplace-dataset-carneiro4"

    def mkid(self, local):
        return '{}_{}'.format('CARNEIRO4', local.rjust(3, '0'))

    @property
    def raw(self):
        return self.raw_dir / '4TH_EDITION'
    ...
    def iter_sources(self):
        ...
        for line in self.raw.joinpath(
                '4th_edition_sources.bib').read_text(encoding='utf8').split('\n'):
            if line.startswith('CARNEIRO4'):
                assert re.match(r'CARNEIRO4_[0-9]{3}_', line)
                if chunk:
                    assert soc
                    yield soc, '\n'.join(chunk), bibids
    ...
    def cmd_makecldf(self, args):
        ...
        for row in self.raw.read_csv(
                '4th_edition_societies.Sheet1.csv', dicts=True):
        ...
        for p in self.raw.joinpath('societies').glob('*.csv'):
            #Trait_ID_4th,Trait_presence,Reference,Original_notes,Comments
            for row in p.parent.read_csv(p.name, dicts=True):
                row = {k: v.strip() for k, v in row.items()}
                if not row['Trait_ID_4th']:
                    continue
                rid = self.mkid(row['Trait_ID_4th'])
                i += 1
                source = []
                refs = {
                ...
                for key, pages in iter_refs(refs):
                    for ok, nk in {
                        ...
                        'Herskovits 1938, vol. 1': ('Herskovits 1938', 'vol. 1'),
                        'Herskovits 1938, vol. 2': ('Herskovits 1938', 'vol. 2'),
                        'Hose and McDougall 1912, vol. 1': ('Hose and McDougall 1912', 'vol. 1'),
                        'Hose and McDougall 1912, vol. 2': ('Hose and McDougall 1912', 'vol. 2'),
                        'Roth 1896, vol. 1': ('Roth 1896', 'vol. 1'),
                        'Roth 1896, vol. 2': ('Roth 1896', 'vol. 2'),
                        'Morgan 1901, vol. 1': ('Morgan 1901', 'vol. 1'),
                        'Morgan 1901, vol. 2': ('Morgan 1901', 'vol. 2'),
                        'Lowie 1935[?]': 'Lowie 1935',
                        'Carcopino 1942': 'Carcopino 1956',
                        'Skeat and Blagden 1906, vol. 1': ('Skeat and Blagden 1906', 'vol. 1'),
                        'Best 1924, vol. 1': ('Best 1924', 'vol. 1'),
                        'Best 1924, vol. 2': ('Best 1924', 'vol. 2'),
                        'Palomeque Torres [?]': 'Palomeque Torres 1948',
                        'Palomeque Palomeque Torres [?]': 'Palomeque Torres 1948',
                        '78-79,198,426': ('Blackwood 1935', '78-79,198,426'),
                        ...
                        if key == ok:
                            ...
                            if pg:
                                if pages:
                                    ...
                                    pages = pg
                    key = {
                        ('Richards 1940', 'CARNEIRO4_019_Bemba.Sheet1'): 'Richards 1940b',
                        ('Skinner 1911', 'CARNEIRO4_046_Menomini.Sheet1'): 'Skinner 1911b',
                        ('Cooper 1946', 'CARNEIRO4_074_Tehuelche.Sheet1'): 'Cooper 1946c',
                        ('Cooper 1946', 'CARNEIRO4_123_Ona.Sheet1'): 'Cooper 1946b',
                        ('Lowie 1912', 'CARNEIRO4_112_Crow.Sheet1'): 'Lowie (1912b)',
                        ('Métraux 1948', 'CARNEIRO4_048_Tupinamba.Sheet1'): 'Métraux (1948c)',
                        ('Métraux 1948', 'CARNEIRO4_058_Guarani.Sheet1'): 'Métraux (1948d)',
                        ('Swanton 1928', 'CARNEIRO4_026_Creek.Sheet1'): 'Swanton 1928d',
                        ('Murdock 1934', 'CARNEIRO4_097_Semang.Sheet1'): 'Murdock 1934d',
                        ('Ames 1959', 'CARNEIRO4_102_Wolof.Sheet1'): 'Ames 1959b',
                        ('Evans-Pritchard 1940', 'CARNEIRO4_060_Nuer.Sheet1'): 'Evans-Pritchard 1940b',
                        ('Best 1925', 'CARNEIRO4_022_Maori.Sheet1'): 'Best 1925b',
                        ('Pospisil 1963', 'CARNEIRO4_109_Kapauku.Sheet1'): 'Pospisil 1963a',
                        ('Handy 1933', 'CARNEIRO4_014_Hawaiians.Sheet1'): 'Handy et al. 1933',
                    }.get((key, p.stem), key)
                    if key in {
                        'Basham 1960',
                        '?',
                        ...
                        'Fitzgerald [?]',
                        '[?]',
                        '',  # 'Harris 1940: 58-59; Lowie 1909: 225,230; ; Lowie 1924: 296'
                        'Driver [?]',
                        'Harley [?]',
                        'Cook [?]',
                        'Vancouver [?]',
